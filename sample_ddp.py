...
def main(args):
    ...
    folder_name = f"Taylor-{model_string_name}-{ckpt_string_name}-size-{args.image_size}-vae-{args.vae}-" \
                  ...
                  f"-{args.interval}-{args.max_order}-{args.use_interp}"
    ...
    for _ in pbar:
        sample_generators = []
        for i in range(n):
            global_sample_index = i * dist.get_world_size() + rank + total
            g = torch.Generator(device=device).manual_seed(args.global_seed + global_sample_index)
            sample_generators.append(g)        
        # Sample inputs:
        z = torch.cat([
            torch.randn(
                (1, model.in_channels, latent_size, latent_size),
                device=device,
                generator=sample_generators[i]
            ) for i in range(n)
        ], dim=0)
        ...
        if args.ddim_sample:
            if args.use_interp:
                while cut_t < total_steps:
                    start_t = cut_t
                    end_t = min(cut_t + interval , total_steps)
                ...
                samples = diffusion.ddim_sample_loop(
                    ...
                    model_kwargs=model_kwargs, progress=False, device=device, generator=sample_generators[i]
                )
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sample-dir", type=str, default="samples2") # Change this to your desired sample directory
