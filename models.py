...
from taylor_utils import derivative_approximation, taylor_formula, taylor_cache_init, interpolate_features
...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, **block_kwargs):
        ...
        self.attn_features_resx = []
        self.block_features_resx = []
    ...
    def forward(self, x, c, current, cache_dic):
        ...
        if current['type'] == 'full':  # Force Activation: Compute all tokens and save them in cache
            ...
            # if not hasattr(self, 'attn_features'):
            #     self.attn_features = []
            # self.attn_features.append(attn_output.detach().cpu())
            ...
            # if not hasattr(self, 'mlp_features'):
            #     self.mlp_features = []
            # self.mlp_features.append(mlp_output.detach().cpu())
        ...
        elif current['type'] == 'interp':  # Force Activation: Compute all tokens and save them in cache
            ...
            elif getattr(self, 'interpolation_mode_coarse', False):
                interp_len = len(self.coarse_feature_attn)
                step_idx = (interp_len - 1) - (current['step'] % interp_len)
                interp_attn = self.coarse_feature_attn[step_idx][current['layer']].to(x.device)
                x = x + gate_msa.unsqueeze(1) * interp_attn

            else: #그냥 forward
                attn_output = self.attn(modulate(self.norm1(x), shift_msa, scale_msa), cache_dic=cache_dic, current=current)
                #derivative_approximation(cache_dic, current, attn_output)
                if getattr(self, 'feature_collection_mode', False):
            ...
            if getattr(self, 'interpolation_mode', False):
                ...
                interp_len = len(self.coarse_feature_mlp)
                step_idx = (interp_len - 1) - (current['step'] % interp_len)
                interp_mlp = self.coarse_feature_mlp[step_idx][current['layer']].to(x.device)
                x = x + gate_mlp.unsqueeze(1) * interp_mlp
                ...
                #derivative_approximation(cache_dic, current, mlp_output)
                if getattr(self, 'feature_collection_mode', False):
                    if not hasattr(self, 'collected_mlp_features'):
                        self.collected_mlp_features = []
                    self.collected_mlp_features.append(mlp_output.detach())
            ...
            #@@@
            #self.attn_features.append(x.detach())
            
            current['module'] = 'mlp'
            x = x + gate_mlp.unsqueeze(1) * taylor_formula(cache_dic, current)
            
            #@@@
            # self.block_features.append(x.detach())
...
class DiT(nn.Module):
    ...
    def forward(self, x, t, current, cache_dic, y): 
        ...
        y: (N,) tensor of class labels
        """
        #print(f"[FORWARD] x input to PatchEmbed:", x.shape)
        #print(f"[FORWARD] {self.__class__.__name__}, x_in={x.shape}")

        x = self.x_embedder(x) + self.pos_embed  # (N, T, D), where T = H * W / patch_size ** 2
        
        #print(f"[FORWARD] after x_embedder={x.shape}, x_embedder type={type(self.x_embedder)}")
        t = self.t_embedder(t)                   # (N, D)
        y = self.y_embedder(y, self.training)    # (N, D)
        c = t + y                                # (N, D)
        ...
        for layeridx, block in enumerate(self.blocks):
            block.feature_collection_mode = getattr(self, 'feature_collection_mode', False)
            block.interpolation_mode = getattr(self, 'interpolation_mode', False)
            block.interpolation_mode_coarse = getattr(self, 'interpolation_mode_coarse', False)
            current['layer'] = layeridx
            if hasattr(self, 'interpolated_attn_features'):
                block.interpolated_attn_features = self.interpolated_attn_features
            if hasattr(self, 'interpolated_mlp_features'):
                block.interpolated_mlp_features = self.interpolated_mlp_features
            if hasattr(self, 'coarse_feature_attn'):
                block.coarse_feature_attn = self.coarse_feature_attn
            if hasattr(self, 'coarse_feature_mlp'):
                block.coarse_feature_mlp = self.coarse_feature_mlp
            #print(f"[BLOCK IN] layer {current['layer']}, x shape = {x.shape}")
    ...
    #def forward_with_cfg(self, x, t, y, cfg_scale):
        ...
        if not hasattr(self, 'pred_noises'):
            self.pred_noises = []
        self.pred_noises.append(half_eps.detach().cpu())
