...
def main(args):
    ...
    torch.save(z, f"shared_z_seed{args.seed}.pt") 
    ...
    if args.ddim_sample:
        if args.use_interp:
            while cut_t < total_steps:
                ...
                #print(f"[WARN] Empty feature list at step {cut_t} (attn={len(model.collected_attn_features)}, mlp={len(model.collected_mlp_features)})")
                if start_t == 0:
                    z_fine = z_next.clone()
                ...
                if end_t <= total_steps - 1 and prev_features is not None:
                    ...
                    # #new_interval = get_interval(start_t)
