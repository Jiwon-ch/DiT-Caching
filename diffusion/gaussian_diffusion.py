...
class GaussianDiffusion:
    ...
    def p_mean_variance(self, model, x, t, current=None, cache_dic=None, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        ...
        if 'use_interp' in model_kwargs:
            current['use_interp'] = model_kwargs['use_interp']
    ...
    def ddim_sample(
        ...
        """
        #print(f"[DDIM_SAMPLE] x_in={x.shape}, t={t}")
        ...
        if generator is not None:
            noise = th.randn(x.shape, device=x.device, dtype=x.dtype, generator=generator)
        else:
            noise = th.randn_like(x)
        ...
        if return_next_latent:
            ...
            nonzero_mask = (t != 0).float().view(-1, *([1] * (len(x.shape) - 1)))
            sample_next = mean_pred + nonzero_mask * sigma * noise
    ...
    def ddim_sample_loop(
        ...
        """
        ###
        end_step = end_step or self.num_timesteps
    ...
    def ddim_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            img = th.randn(*shape, device=device)
        #print(f"[DEBUG] num_timesteps={self.num_timesteps}, start={start_step}, end={end_step}")
        indices = list(range(self.num_timesteps))[::-1]
        if end_step is None:
